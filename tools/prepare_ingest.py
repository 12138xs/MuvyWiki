...
def manifest_match_by_path(entries: list[dict[str, Any]], path: str) -> dict[str, Any] | None:
    for entry in entries:
        if path in {entry.get("raw_path"), entry.get("converted_path"), entry.get("converted_from")}:
            return entry
    return None
...
def prepare_one(
    ...
    id_or_hash_match = manifest_match_by_id_or_hash(entries, source_id, content_hash)
    path_match = manifest_match_by_path(entries, rel_path) if input_kind == "raw-converted" else None
    manifest_match = path_match or id_or_hash_match
    duplicate = manifest_match is not None
    if manifest_match is not None:
        status = "blocked"
        exit_code = max(exit_code, 1)
        warnings.append("Duplicate source_id, content_hash, or converted path found in raw/source-manifest.jsonl.")

    if input_kind == "raw-converted":
        raw_path_value = manifest_match.get("raw_path") if manifest_match else None
